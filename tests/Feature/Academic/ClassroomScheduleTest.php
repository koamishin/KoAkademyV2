...
test('a room cannot be double booked', function (): void {
    ...
    $classOffering = ClassOffering::query()->create(['campus_id' => $campus->id, 'term_id' => $term->id, 'subject_id' => $subject->id, 'name' => 'Math A', 'code' => 'MATH-A']);
    $target = ClassOffering::query()->create(['campus_id' => $campus->id, 'term_id' => $term->id, 'subject_id' => $subject->id, 'name' => 'Math B', 'code' => 'MATH-B']);
    ClassMeeting::query()->create(['class_offering_id' => $classOffering->id, 'room_id' => $room->id, 'day_of_week' => 1, 'starts_at' => '08:00', 'ends_at' => '09:00']);
