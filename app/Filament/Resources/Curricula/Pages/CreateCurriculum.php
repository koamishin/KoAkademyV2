...
final class CreateCurriculum extends CreateRecord
{
    use HasWizard;
            ...
            Step::make('School context')
                ...
                ->schema([
                    Section::make('Where will this curriculum be used?')
                        ->description('The institution, campus, and available academic levels follow the current campus configuration.')
                        ...
                        ->schema([
                            ...
                            Select::make('education_level_id')
                                ...
                                ->options(fn (): array => CurriculumResource::configuredEducationLevelOptions($this->campus()))
                                ->default(fn (): int|string|null => array_key_first(CurriculumResource::configuredEducationLevelOptions($this->campus())))
                                ->helperText('Only levels configured for this campus are available.')
                                ->searchable()
                                ->preload()
                                ->live()
                                ->required()
    ...
    private function templateOptions(Get $get): array
    {
        $level = EducationLevel::query()
            ...
            ->whereKey(CurriculumResource::configuredEducationLevelIds($this->campus()))
            ->first();
