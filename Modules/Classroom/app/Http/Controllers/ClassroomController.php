...
final class ClassroomController extends Controller
{
    public function index(CurrentCampus $currentCampus): Response
    {
        ...
        return Inertia::render('classroom/Index', ['classes' => ClassOffering::query()->where('campus_id', $currentCampus->id())->where(fn ($q) => $q->where('teacher_id', $person?->id)->orWhereHas('members', fn ($m) => $m->where('person_id', $person?->id)->where('status', 'active')))->with(['teacher:id,first_name,last_name'])->latest()->get()]);
    }

    public function show(ClassOffering $classOffering, CurrentCampus $currentCampus): Response
    {
        $person = request()->user()->person;
        abort_unless($classOffering->campus_id === $currentCampus->id(), 404);
